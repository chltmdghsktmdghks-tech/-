...
/// **악기별 음량 맞추기.** 녹음마다 레벨이 딴판이라 그대로 쓰면 한 밴드가 안 된다.
///
/// `test/sample_level_meter.dart` 로 **각 악기를 제 음역대에서** 쳐서 잰 값이다
/// (베이스를 C4 로 재면 원래 안 나는 소리를 늘려 재는 셈이라 값이 엉뚱해진다).
/// 2026-09-22 실측, 가장 센 0.3초 구간의 RMS:
///
///   sax −7.7 · epiano −12.4 · guitar −13.9 · piano −15.3 · trumpet −17.1
///   cello −17.7 · fingerbass −21.1 · violin −25.7 · **upright −41.0** dBFS
///
/// **33.3dB 차이**다. 재즈에서 색소폰만 귀가 아프게 크고 더블베이스는 있는지도
/// 모르던 것이 이것이다 — 곡에 바닥이 없으니 "싸구려 미디"로 들린다.
/// (`genre_mix.dart` 의 jazz `kStyleGain` 1.72 도 안 들리는 업라이트를 메꾸려고
/// 올려 둔 값이라, 이 표를 넣은 뒤 같이 다시 재야 한다)
///
/// ── **RMS 가 아니라 피크로 맞춘다** ──
///
/// 처음엔 RMS 를 맞췄다가 `test/zz_peak_probe` 로 재 보고 되돌렸다:
/// 뜯는 악기는 어택만 뾰족하고 뒤가 조용해서 RMS 가 낮다(핑거베이스는 피크
/// 대비 RMS 가 25dB 아래다). 그걸 사스폰 같은 **지속음**에 RMS 로 맞추면
/// 피크가 천장을 뚫는다 — 실제로 핑거베이스가 **+11.0dB** 로 넘쳤다.
///
/// 그래서 **한 음의 피크가 −6dBFS** 에 오게 맞춘다. 악기마다 다른 피크-RMS
/// 성격(크레스트)이 그대로 남으므로, 뜯는 베이스는 여전히 RMS 로는 작게
/// 잡힌다 — 그게 음악적으로 맞다(뜯은 음은 지속음이 아니라 한 방이다).
/// −6dB 를 남기는 것은 화음으로 여러 음이 겹칠 자리다.
///
/// **재는 쪽도 함정이 있었다**: 처음엔 왼쪽 채널만 쟀는데, 오른쪽으로 패닝된
/// 악기(바이올린 +0.35 · 기타 +0.2 · 전자피아노 +0.1)는 오른쪽이 더 크다.
/// 그래서 그 셋만 천장 가까이(바이올린 −0.9dB) 올라가 있었다.
/// `test/sample_level_test.dart` 는 **양쪽 채널의 최대**로 잰다.
const Map<String, double> kSampleTrim = {
  'upright': 14.06, // 원본 피크가 −29dB — 이 팩만 정규화가 안 돼 있다
  'violin': 1.37,
  'cello': 1.17,
  'trumpet': 0.89,
  'piano': 0.86,
...
///
/// 실측 ff−pp: 첼로 25.4 · 트럼펫 23.6 · 피아노 23.4 · 바이올린 22.4 ·
/// 핑거베이스 20.2 dB. 여기에 신스용 세기표(`VG`, 0.30/0.60/1.0 = −10.5dB)를
/// **또 곱하면** 여리게가 −43dB 까지 내려가 아예 안 들린다 — 리듬이 딱딱하게
/// 들리던 이유다. 그래서 이 악기들만 완만한 표를 쓴다.
///
/// 나머지(기타 5.6 · 전자피아노 5.9 · 색소폰 8.7 · 업라이트 10.4 dB)는 세기
/// 층 절반이 사실 **같은 파일**이라(md5 확인) 녹음이 세기를 안 담고 있다.
/// 그쪽은 기존 `VG` 를 그대로 써야 세기 차이가 남는다.
const Set<String> kRealVelSamples = {
  'piano',
  'cello',
  'violin',
  'trumpet',
...
/// [kRealVelSamples] 에 쓰는 완만한 세기 배수 (−1.9dB 폭).
const Map<int, double> kSampleVG = {1: 0.80, 2: 0.90, 3: 1.0};
