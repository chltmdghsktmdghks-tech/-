...
class FlutterPcmSound {
  ...
  static Future<void> feed(PcmArrayInt16 buffer) async {
    ...
    // 음악 낙서장 수정: **뷰(offset/length)를 지켜서** 보낸다.
    // 원본은 `bytes.buffer.asUint8List()` 라 밑에 깔린 버퍼 **전체**를 보냈다.
    // 그래서 버퍼를 재사용하려고 앞쪽 일부만 담은 뷰를 넘기면 뒤쪽 쓰레기까지
    // 같이 재생됐다. 뷰를 지키면 렌더 버퍼를 한 개만 두고 돌려쓸 수 있다.
    final b = buffer.bytes;
    return await _invokeMethod('feed',
        {'buffer': b.buffer.asUint8List(b.offsetInBytes, b.lengthInBytes)});
  }

  /// 음악 낙서장 추가: 지금 큐에 남아 있는 프레임 수를 직접 물어본다.
  /// 백그라운드 아이솔레이트에서는 feed 콜백을 받을 수 없으므로 이게 필요하다.
  static Future<int> remainingFrames() async {
    final v = await _invokeMethod<int>('remainingFrames');
    return v ?? 0;
  }
